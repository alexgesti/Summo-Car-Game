...
ModulePlayer2::ModulePlayer2(bool start_enabled) : Module(start_enabled), vehicle(NULL)
{
...
update_status ModulePlayer2::Update(float dt)
{
	...
	if (live > 0)
	{
		...
		if (counter_speed_go_go >= 2.5f && counterstart)
		{
			...
			App->scene_intro->GoFuckingNuts2.color = Red;
		}
		...
		if (counter_cant_go_speed >= 10 && countercantspeed)
		{
			...
			counter_cant_go_speed = 0;
			App->scene_intro->GoFuckingNuts2.color = Green;
		}
	...
	if (App->input->GetKey(SDL_SCANCODE_F4) == KEY_DOWN) live--;
